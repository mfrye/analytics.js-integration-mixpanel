...
/**
 * Trait aliases.
 * Including an array of trait values so we can extend the peopleProperties array to easily include Mixpanel's
 * special traits
 */
...
Mixpanel.prototype.identify = function(identify) {
  ...
  var peopleProps = pick(extend(traitValues, opts.peopleProperties) || [], traits);
...
Mixpanel.prototype.track = function(track) {
  ...
  var peopleProps = pick(extend(traitValues, opts.peopleProperties) || [], props);
  ...
  if (!is.empty(peopleProps) && people) {
    // add Mixpanel's special props into peopleProps Obj
    for (var prop in props) {
      if (traitAliases.hasOwnProperty(prop)) peopleProps[traitAliases[prop]] = props[prop];
    }
